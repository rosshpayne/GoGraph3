...
func (r *RootStmt) MarshalJSON() string {
	...
	for i, uid := range uids {
		...
		nvc := r.nodesc[uuid.UIDstring(uid)]
		nvm := r.nodes[uuid.UIDstring(uid)]
			...
			case *UidPred: // child of child, R.N.N
				...
				for i, uids := range upred.Value.([][][]byte) {
					for j, v := range uids {
						...
						// s.WriteString(fmt.Sprintf("%suid: %s\n", strings.Repeat("\t", 2), uuid.UID(v).String()))
	...
	for k, s := range upred.Select {
		...
		case *UidPred: // child of child, R.N.N
			...
			for i, uids := range upred_.Value.([][][]byte) {
				for j, v := range uids {
					//fmt.Printf("i, j, UID: %d %d, %s", i, j, uuid.UID(v).String())
					...
					// s.WriteString(fmt.Sprintf("%suid: %s\n", strings.Repeat("\t", u.lvl+1), uuid.UID(v).String()))
