...
func Propagate(limit *grmgr.Limiter, wg *sync.WaitGroup, pUID uuid.UID, ty string, has11 map[string]struct{}) {
	...
	for _, v := range tyAttrs {
		...
		psortk := concat(types.GraphSN(), "|A#", dpPart, "#:", v.C)
		//	syslog(fmt.Sprintf("Propagate top loop : pUID %s ,   Ty %s ,  psortk %s ", pUID.String(), v.Ty, psortk))
		...
		if err != nil {
			...
			if errors.Is(err, NoDataFound) {
				syslog(fmt.Sprintf("No items found for pUID:  %s, sortk: %s ", pUID, psortk))
				fmt.Printf("XXXX No items found for pUID:  %s, sortk: %s\n", pUID, psortk)
				err = nil
				break
			}
			elog.Add("dp: ", err)
			...
			go func(oid int, rch <-chan cache.BatchPy, v blk.TyAttrD) {
				...
				for py := range rch {
					...
					go func(py cache.BatchPy, v blk.TyAttrD) {
						...
						// ptx := tx.New(concat(pUID.String(), "-", strconv.Itoa(oid)))
						// fmt.Println("****************** ptx label: ", concat(pUID.String(), "-", strconv.Itoa(oid)))
