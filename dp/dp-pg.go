...
func Propagate(limit *grmgr.Limiter, wg *sync.WaitGroup, pUID uuid.UID, ty string, has11 map[string]struct{}) {
	...
	var (
		...
		err         error
		wgc         sync.WaitGroup
		NoDataFound = db.NoDataFound
		b           bool
	)

	syslog(fmt.Sprintf("Enter DP: pUID %s ", pUID.Base64()))
	...
	for _, v := range tyAttrs {
		...
		syslog(fmt.Sprintf("Propagate Start: pUID %s ,   Ty %s ,  psortk %s ", pUID.Base64(), v.Ty, psortk))
		...
		if err != nil {
			...
			elog.Add("dp", fmt.Errorf("FetchForUpdate error for pUID:  %s, sortk: %s  %w", pUID.Base64(), psortk, err))
			break
		}
		...
		if err != nil {
			elog.Add("MakeChannel", fmt.Errorf("Puid error: %s %s", pUID.Base64(), err))
			break
		}
		...
		// create a goroutine to read from each channel created in MakeChannels()
		// there will be one goroutine to handle the uid-pred (index 0) in the node and one goroutine for each overflow block
		for i, k := range bChs {
			...
			go func(oid int, rch <-chan cache.BatchPy, v blk.TyAttrD) {
				...
				if oid == 0 {
					blimiter = nil // embedded uids in UID-PRED
				} else {
				...
				for py := range rch {
					...
					// batch goroutine - enables concurrent processing of batches (embedded and overflow) within a payload
					go func(py cache.BatchPy, v blk.TyAttrD) {
						...
						pUID_ := py.Puid // either parent node or overflow UID
							...
							for _, t := range types.TypeC.TyC[v.Ty] {
								...
								for k, m := range ncc.GetMap() {
									...
									if k == sk_ {
										...
										merge := ptx.MergeMutation(tbl.EOP, pUID_, psk, mutdml)
									...
									for k, m := range ncc.GetMap() {
										...
										if k == compare {
											...
											case "S":
												...
												nv := make([]bool, 1, 1)
												nv[0] = bl[0]
												ptx.MergeMutation(tbl.EOP, pUID_, psk, mutdml).AddMember("LS", v).AddMember("XBl", nv)
											...
											case "I":
												...
												nv := make([]bool, 1, 1)
												nv[0] = bl[0]
												ptx.MergeMutation(tbl.EOP, pUID_, psk, mutdml).AddMember("LI", v).AddMember("XBl", nv)
											...
											case "F":
												...
												nv := make([]bool, 1, 1)
												nv[0] = bl[0]
												ptx.MergeMutation(tbl.EOP, pUID_, psk, mutdml).AddMember("LF", v).AddMember("XBl", nv)
											...
											case "B":
												...
												nv := make([]bool, 1, 1)
												nv[0] = bl[0]
												ptx.MergeMutation(tbl.EOP, pUID_, psk, mutdml).AddMember("LB", v).AddMember("XBl", nv)
											...
											case "Bl":
												...
												nv := make([]bool, 1, 1)
												nv[0] = bl[0]
												ptx.MergeMutation(tbl.EOP, pUID_, psk, mutdml).AddMember("LBl", v).AddMember("XBl", nv)
											}
											ptxlk.Unlock()
										}
									}
		...
		// not all Performances edges are connected - e.g. Person Director usually will have no Actor(Performance) edges
		// if !ptx.HasMutations() {
	...
	syslog(fmt.Sprintf("Exit for pUID:  %s", pUID.Base64()))
}

func FetchNodeCh(ctx context.Context, ty string, stateId uuid.UID, restart bool) <-chan uuid.UID {
...
func ScanForDPitems(ctx context.Context, ty string, dpCh chan<- uuid.UID, id uuid.UID, restart bool) {
	...
	//
	// Option 2: replace index query with a paginated index query.
	//
	// 1. Paginated query on index. Use page size of 200 items (say)
	// 2. DP it.
	// 3. Go To 1.
	//
	...
	slog.Log(logid, fmt.Sprintf("type %q started. paginate id: %s. restart: %v", ty, id.Base64(), restart))

	rec := []Unprocessed{}

	ptx := tx.NewQueryContext(ctx, "dpScan", tbl.Block, "TyUID")
	if err != nil {
		elog.Add(logid, err)
		return
	}

	ptx.Select(&rec).Key("Ty", types.GraphSN()+"|"+ty).Key("IX", "X").Limit(param.DPbatch).Paginate(id, restart)
	slog.Log(logid, fmt.Sprintf("Key TY %q  ", types.GraphSN()+"|"+ty))

	for !ptx.EOD() {

		slog.Log(logid, "Execute...")
		...
		for _, v := range rec {
			slog.Log(logid, fmt.Sprintf("fetched PKey: %s ", v.PKey.Base64()))
			dpCh <- v.PKey
		}
	}
}
